...
int main(){
    ...
    for(int tc = 0; tc < 10; tc++){
        ...
        shuffle(mst_edges.begin(), mst_edges.end(), rng);
        bool can_be_done = gen0(1);
        if(can_be_done){
            p = gen1(n-1);
            for(int i = 0; i < p; i++){
                compulsory.pb({mst_edges[i][1], mst_edges[i][2]});
            }
        }else{
            while(1){
                compulsory.clear();
                p = gen1(m);
                for(int i = 0; i < p; i++){
                    compulsory.pb({edges[i][0], edges[i][1]});
                }
                if(!solve(n, edges, compulsory)) break;
            }
        }
        input << n << " " << p << " " << m << "\n";
