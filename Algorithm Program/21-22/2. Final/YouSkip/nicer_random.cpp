...
int main(){
    for(int tc = 0; tc < 5; tc++){
        ...
        vector<vector<ll>> dist_nodes;
        for(int i = 1; i <= n; i++) dist_nodes.pb({dist[i][s], i});
        RSORT(dist_nodes);
        vector<int> final_nodes;
        for(auto p: dist_nodes){
            if(p[0] > INT_MAX){
                final_nodes.pb(p[1]);
                k++;
            }
        ...
        assert(k);
        k = gen(k);
        final_nodes = vector<int>(final_nodes.begin(), final_nodes.begin() + k);
        assert(k);
        ll min_dist = 1e18;
