...
def randints_with_target_sum(n: int, target_sum: int, lower_bound: int) -> List[int]:
    ...
    assert (
        n * lower_bound <= target_sum
    ), f"Unable to generate, the minimum possible sum is greater than the target sum"
...
def gen_tree(n: int, parent_dist: int = 10**9, root: int = 1) -> List[Tuple[int, int]]:
    ...
    for node_index, node in enumerate(nodes):
        ...
        if not node_index:
            continue
        parent_index = random.choice(
            range(max(0, node_index - parent_dist), node_index)
        )
...
def make_graph_weighted(
    edges: list[tuple[int, int]], l: int, r: int
) -> list[tuple[int, int, int]]:
    """Makes the graph weighted by assigning random weights to the edges.

    Args:
        edges (list[tuple[int, int]]): The list of edges.
        l (int): The lower bound of the weight.
        r (int): The upper bound of the weight.

    Returns:
        list[tuple[int, int, int]]: The list of weighted edges.
    """

    return [(u, v, randint(l, r)) for u, v in edges]
