...
class GeneratorSystem:
    ...
    def __generateInputFiles(self) -> None:
        ...
        for inputIndex, input in enumerate(tqdm(inputs)):
            with open(
                ...
                inputFile.write(f"{input.n} {input.m} {input.q}\n")
                for edge in input.edges:
                    inputFile.write(f"{edge[0]} {edge[1]} {edge[2]}\n")
                inputFile.write(
                    "\n".join(
                        [f"{query[0]} {query[1]} {query[2]}" for query in input.queries]
                    )
                )
    ...
    def __compileSolver(self) -> None:
        subprocess.run(["g++", "-Wl,--stack,1000000000", "sol.cpp", "-O2"])
