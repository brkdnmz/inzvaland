...
def update(cur, l, r, target, op, flipped):
    if not(l <= target <= r):
        return
    if l == r:
        if op == 0:
    ...
    mid = (l+r)//2
    update(2*cur, l, mid, target, op, flipped)
    update(2*cur+1, mid+1, r, target, op, flipped)
...
for tc in range(5):
    ...
    for i in range(q-1):
        ...
        if type == 1:
            i = randint(0, N)
            update(1, 0, N, i, 0, flipped)
            input.write("1 " + str(i) + "\n")
        elif type == 2:
            i = randint(0, N)
            update(1, 0, N, i, 1, flipped)
            input.write("2 " + str(i) + "\n")
        elif type == 3:
            flipped = not flipped
            input.write("3\n")
