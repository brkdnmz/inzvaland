...
indicated_prob = random() * 1e-4
t = 1
NO = 15
SET_NAME = f"big few pixels"
N_TC = 5
...
def generate_test_cases():
    ...
    def gen_grid(n: int, m: int, prob: float) -> List[str]:
        l, r = 0, m - 1  # sorted(list(np.random.choice(range(m), 2)))
        u, d = 0, n - 1  # sorted(list(np.random.choice(range(n), 2)))
    ...
    ns: List[int] = [2 * x for x in gen_ints_sum_n(tot_rows // 2, t)]
    ms: List[int] = [2 * x for x in gen_ints_sum_n(tot_cols // 2, t)]
    grids: List[List[str]] = []
    anss: List[int] = []

    assert len(ns) == t and min(ns) > 1 and sum(ns) <= BOUND
    assert len(ms) == t and min(ms) > 1 and sum(ms) <= BOUND
